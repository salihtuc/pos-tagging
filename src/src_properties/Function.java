...
public class Function implements DifferentiableFunction {
	...
	public static double LAMBDA_EM = 1.0;
	...
	public double[] gradient(double[] iterWeights) {
		...
		for (int i = 0; i < Main.tagSize; i++) {
			for (int j = 0; j < Main.tagSize; j++) {
						...
						for (int k = 1; k < lattice.size() - 1; k++) {
							...
							for (int k1 : n.next) {
								...
								if (n2.isEndState) { // if node end state calculate initial i-</s>
									valuesInitEnd.add(Main.initialProbabilities.get(Main.tagList.get(i) + "|</s>")); // calculate
																														// initial
																														// i-</s>
									for (int k2 = 0; k2 < Main.tagSize; k2++) {
										valuesInitEndDenom
												.add(Main.initialProbabilities.get(Main.tagList.get(k2) + "|</s>")); // calculate
																														// initial
																														// all
																														// tags-</s>
									}
							...
							if (startStates.contains(n.stateNum)) { // if node start state calculate initial <s>-i
								...
								valuesInit.add(emission + Main.initialProbabilities.get("<s>|" + Main.tagList.get(i))); // calculate
																														// initial
																														// <s>-i
								for (int k2 = 0; k2 < Main.tagSize; k2++) {
									...
									valuesInitDenom.add(
											emission2 + Main.initialProbabilities.get("<s>|" + Main.tagList.get(k2))); // calculate
																														// initial
																														// <s>-
																														// all
																														// tags
				...
				orgScore += divide(transOriginalNum, transOriginalDenom); // divide for all sentences transOriginalNum /
																			// transOriginalDenom
				negScore += divide(transNeighborNum, transNeighborDenom); // divide for all negative sentences
																			// transNeighborNum / transNeighborDenom
